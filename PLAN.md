...
| vyperHoareExample2Script.sml | example_2_thm | If-statement proof incomplete - uses stmts_spec_if with TL scopes |

### Helper Lemmas Proven

The following helper lemmas have been added and PROVEN in vyperHoareExample2Script.sml:

**Basic helper lemmas:**
- `lookup_scoped_var_implies_var_in_scope`: Lookup SOME implies var_in_scope
- `var_in_scope_after_update`: var_in_scope holds after update for the updated variable
- `var_in_scope_preserved_after_update`: var_in_scope preserved for other variables after update
- `lookup_scoped_var_TL_when_not_in_HD`: Lookup in full scopes equals lookup in TL when HD doesn't contain the var
- `lookup_scoped_var_from_TL_when_not_in_HD`: Reverse direction
- `var_in_scope_from_TL_when_not_in_HD`: var_in_scope propagates from TL to full when not in HD
- `valid_lookups_from_TL_when_FEMPTY_HD`: valid_lookups propagates from TL to full when HD=FEMPTY
- `valid_lookups_preserved_existing_update`: valid_lookups preserved when updating EXISTING variable (not new)
- `find_containing_scope_none_lookup_scopes_none`: Links find_containing_scope NONE to lookup_scopes NONE

**Key lemmas for if-statement branches (HD=FEMPTY pattern):**
- `update_scoped_var_TL_when_FEMPTY_HD`: TL of update equals update in TL when HD=FEMPTY
- `lookup_scoped_var_TL_after_update_when_FEMPTY_HD`: Lookup in TL after update returns new value
- `lookup_scopes_FEMPTY_CONS`: lookup_scopes ignores FEMPTY at head
- `lookup_scoped_var_FEMPTY_CONS`: lookup_scoped_var with FEMPTY pushed equals original
- `lookup_scoped_var_TL_FEMPTY_CONS`: TL of FEMPTY::rest gives back rest
- `var_in_scope_FEMPTY_CONS`: var_in_scope with FEMPTY pushed equals original
- `valid_lookups_FEMPTY_CONS`: valid_lookups propagates through FEMPTY push
- `var_in_scope_from_TL_FEMPTY`: var_in_scope propagates from TL when HD=FEMPTY
- `valid_lookups_from_TL_FEMPTY`: valid_lookups propagates from TL when HD=FEMPTY
- `lookup_TL_after_update_FEMPTY`: Lookup in TL after update returns new value (when HD=FEMPTY)
- `scopes_nonempty_TL_after_update_FEMPTY`: TL of scopes remains nonempty after update
- `valid_lookups_TL_after_update_FEMPTY`: valid_lookups preserved in TL view after update
- `lookup_other_TL_after_update_FEMPTY`: Other variable lookup preserved in TL view after update

### Current Work: example_2_thm If-Statement Proof

The example_2_thm proof has 3 statements:
1. ✓ AnnAssign "x" (Name "x_arg") - PROVEN using stmts_spec_ann_assign
2. ✓ AugAssign (NameTarget "x") Add (Literal 10) - PROVEN using stmts_spec_aug_assign_scoped_var
3. ⚠ If statement - IN PROGRESS with cheat

**The If-Statement Challenge:**

The `stmts_spec_if` rule works with `st with scopes := TL st.scopes` views because:
- If-statement pushes FEMPTY scope on entry
- Branch body executes with pushed scope
- Scope is popped on exit

The precondition in branches is `λst. P (st with scopes := TL st.scopes) ∧ condition`.
The postcondition is `λst. Q (st with scopes := TL st.scopes)`.

**Progress on if-statement proof (exploratory session):**

1. ✓ Case split on `xarg + 10 > 100` works correctly
2. ✓ Using `if_inst_T = ISPECL [..., BoolV T] stmts_spec_if` for the then-branch case
3. ✓ Condition expression proof: Uses expr_spec_builtin_bop, expr_spec_scoped_var, expr_spec_literal
4. ✓ Else-branch with F precondition: trivially proven with `simp[stmts_spec_def]`
5. ⚠ Then-branch (x := 100): Needs to work with TL-scopes pattern

**Blocking issue:**

The `stmts_spec_assign_scoped_var` rule expects normal state `st`, but the if-branch precondition/postcondition
are written as `st with scopes := TL st.scopes`. Need either:

Option A: Add a specialized rule `stmts_spec_assign_in_if_branch` to vyperHoareScript.sml that:
- Works with `st with scopes := TL st.scopes` view in pre/postconditions
- Uses knowledge that `HD st.scopes = FEMPTY` inside branches

Option B: Move `stmts_spec_if_FEMPTY` (which exposes `HD st.scopes = FEMPTY`) to vyperHoareScript.sml
where internal lemmas like `with_scopes_id`, `scopes_cons_lemma`, `eval_stmts_preserves_scopes_len` are available.

Option C: Export the needed internal lemmas from vyperHoareScript.sml so they can be used in example proofs.

**Recommended next step:** Option B - add stmts_spec_if_FEMPTY to vyperHoareScript.sml with the proof
that uses internal lemmas, then use it in example_2_thm

...
Vyper-HOL releases are GitHub Releases. Each release should include a prebuilt
holbuild archive for the project default roots.
...
2. Ensure the `holbuild` workflow is green for the commit being released. It
   should have uploaded an artefact named `vyper-hol-hbx-<commit-sha>`.
...
4. If needed, run the `holbuild` workflow manually for the tag and wait for it
   to upload `vyper-hol-hbx-<commit-sha>`.
...
holbuild export \
  -o vyper-hol.hbx \
  --metadata-out vyper-hol.hbx.json
sha256sum vyper-hol.hbx > vyper-hol.hbx.sha256
```
...
gh release download vX.Y.Z \
  -p vyper-hol.hbx \
  -p vyper-hol.hbx.json \
  -p vyper-hol.hbx.sha256
sha256sum -c vyper-hol.hbx.sha256
holbuild import vyper-hol.hbx
holbuild build
```

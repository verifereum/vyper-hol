...
Proof
  ...
  res_tac >> gvs[] >> ntac 2 (first_x_assum drule >> simp[])
QED
...
Proof
  rpt strip_tac >>
  gvs[evaluate_def, bind_def, AllCaseEqs(), return_def, raise_def, ignore_bind_def, lift_option_def] >>
  pop_assum mp_tac >> pop_assum mp_tac >>
  qid_spec_tac `st` >> qid_spec_tac `s''` >> qid_spec_tac `vs` >>
  Induct_on `MAP SND kes` >> simp[evaluate_def, return_def] >>

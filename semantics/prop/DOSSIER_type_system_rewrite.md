...
- `E0163` (progressed, tool_limit)
  - Rewrote well_typed_binop_no_type_error proof: (1) moved evaluate_type_BaseT_SOME and added evaluate_type_FlagT_SOME bridge lemmas before main theorem; (2) replaced Cases_on `ty = BaseT DecimalT` with Cases_on `ty` >> gvs[] for all cases; (3) used imp_res_tac evaluate_type_BaseT_SOME for type resolution; (4) added FlagT subgoals for And/Or/XOr/Eq; (5) added t2 Cases_on for ShL/ShR -> Three build attempts with different tactic patterns: (a) fs[evaluate_type_BaseT_SOME] after Cases_on→type var not substituted; (b) gvs[evaluate_type_BaseT_SOME]→simplifier can't match bare-variable LHS conclusion; (c) imp_res_tac evaluate_type_BaseT_SOME after Cases_on `ty` >> gvs[] >> Cases_on `b` >> gvs[]→works for type/value resolution but branch tail count error from holbuild proof runtime on `>- ... >> gvs[]` structure ()
- `E0164` (stuck, risk_mismatch)
  - Cases_on bop >- (fs[well_typed_binop_def] >> Cases_on ty >> gvs[] >> Cases_on b >> gvs[] >> imp_res_tac evaluate_type_BaseT_SOME >> gvs[] >> simp[evaluate_binop_def]) -> gvs[] destroys no-TypeError !msg conclusion; gvs eliminates msg variable producing unsolvable F goal. Also >- (inner) >> gvs[] causes branch tail count error when inner fully solves. (`tool_output:TO_type_system_rewrite-20260516T153850Z_m23247_t001`, `tool_output:TO_type_system_rewrite-20260516T153850Z_m23249_t001`)
  - Replace gvs[] with fs[], flat >> chains instead of >- ... >> tail. Also evaluate_type_BaseT_SOME via imp_res_tac for type resolution. -> Proof keyword accidentally deleted from theorem. Parse error. fs[] approach not yet tested since build failed on parse error first. (`tool_output:TO_type_system_rewrite-20260516T153850Z_m23279_t001`)
- `E0165` (progressed, tool_limit)
  - Removed duplicate cheat well_typed_binop_no_type_error (line 654). Fixed 3 parse errors: stray } in And/Or/XOr flag-type subcases (lines 945,970,995). Changed metis_tac[is_int_type_inv] to metis_tac[is_int_type_imp] in numeric_same_type_binop_resolved and int_same_type_binop_resolved helper lemmas. Attempted to add forward-implication lemmas (is_int_type_imp, is_flag_type_imp, is_comparable_type_imp) but disjunction syntax got corrupted by API - the ∨ operator between existential terms was lost, causing type error (function application instead of disjunction). -> File is in BROKEN state: (1) is_int_type_imp is referenced but not defined (broken lemma removed after syntax failure), (2) 10 remaining metis_tac[is_int_type_inv] calls in main proof still need to be replaced with is_int_type_imp (`tool_output:TO_type_system_rewrite-20260516T153850Z_m23523_t001`)
- `E0166` (progressed, bad_definition)
  - Added forward-implication lemmas is_int_type_imp, is_flag_type_imp, is_comparable_type_imp. Replaced all metis_tac[is_int_type_inv] forward-usage calls with metis_tac[is_int_type_imp] (failed), then first_x_assum ACCEPT_TAC MATCH_MP (works for ∃-disjunction derivation). Rewrote numeric_same_type_binop_resolved and int_same_type_binop_resolved helper proofs to use Cases_on ty >> Cases_on b >> simp[is_int_type_def] instead of metis_tac[is_int_type_inv]. Changed drule to drule_all for consuming helpers. -> Forward-implication lemmas proved. ACCEPT_TAC+MATCH_MP works for ∃-disjunction derivation. Helper proof rewrites succeeded. But drule_all>>strip_tac on helpers with ∃-quantified conclusions has critical variable scoping bug: v2 gets both v1's and v2's bindings. The main theorem Add/Sub/Mul/Div cases still fail because of this bug. Need to abandon helper+drule approach and use Cases_on or gvs+Excl directly. ()
- `E0167` (progressed, bad_definition)
  - Removed value_has_type antecedents and value conclusions from numeric_same_type_binop_resolved and int_same_type_binop_resolved helpers, converting them to type-only resolution lemmas. Old helpers had ∃n i1 i2. ... v1 = IntV i1 ∧ v2 = IntV i2 which caused variable scoping bugs under drule_all>>strip_tac. New helpers have ∃n. ... ty = BaseT(UintT n) ∧ result_tv = BaseTV(UintT n) ∧ tv1 = BaseTV(UintT n) ∧ tv2 = BaseTV(UintT n) with no value variables. Main theorem consumers NOT yet updated. -> Helpers modified but main theorem well_typed_binop_no_type_error not yet updated to consume new helper shapes. Build will fail until consumer cases are rewritten to add value resolution after type resolution from helpers. ()
- `E0168` (progressed, tool_limit)
  - Abandoned helper lemma approach (numeric_same_type_binop_resolved, int_same_type_binop_resolved). Rewrote well_typed_binop_no_type_error with fully inline type+value resolution per case: (1) is_numeric_type_inv/is_int_type_imp for type disjunction, (2) ACCEPT_TAC+MATCH_MP for ∃-disjunction derivation, (3) evaluate_type_BaseT_SOME for type_value resolution, (4) mp_tac >> simp >> strip_tac >> simp for value resolution, (5) irule for per-operator boundary lemma. Removed helper lemmas entirely. NOT YET BUILT. -> File edited with complete inline proof for all 22 binop cases. Helper lemmas removed. Build not yet run to verify. (`tool_output:TO_type_system_rewrite-20260516T153850Z_m23701_t002`, `tool_output:TO_type_system_rewrite-20260516T153850Z_m23716_t001`)
- `E0169` (progressed, tool_limit)
  - Replace qspec_then with qx_choose_then for existential assumptions in well_typed_binop_no_type_error proof -> qx_choose_then also fails: after `by` on disjunctions with existentials, STRIP_ASSUME_TAC already destructs the ∃ witnesses, leaving nothing for first_x_assum to find. FIRST_ASSUM exception. (`tool_output:TO_type_system_rewrite-20260516T153850Z_m23832_t001`)
  - Structural rewrite: replace disjunction+existential pattern with Cases_on `ty` >> Cases_on `b` approach. gvs[is_numeric_type_def, is_int_type_def] safely kills impossible cases after Cases_on (concrete types). Valid cases get named n/m variables directly from Cases_on. -> Complete proof rewritten for all 22 binop cases. NOT YET BUILT - file edited but holbuild not run to verify. ()
- `E0170` (progressed, other)
  - gvs[well_typed_binop_def, Excl is_*_type_def, is_*_type_inv, AllCaseEqs(), evaluate_type_BaseT_SOME] >> TRY irule per-operator boundary lemmas >> simp[Once evaluate_binop_def] >> rpt IF_CASES_TAC >> simp[] for Div/Mod/Exp >> drule vht_ArrayTV_exists >> strip_tac >> gvs[] for In/NotIn array cases -> Down to 2 remaining goals before accidentally deleting helper lemmas. gvs+Excl approach works perfectly: resolves all type classifiers and value constructors, creates 86 goals that close by irule. Div/Mod/Exp need IF_CASES_TAC. In/NotIn need vht_ArrayTV_exists drule. (`TO_type_system_rewrite-20260516T153850Z_m23982_t001`, `TO_type_system_rewrite-20260516T153850Z_m23997_t001`)
- `E0171` (progressed, tool_limit)
  - Fixed well_typed_binop_def: UAdd/USub/UMul/UDiv → UnsafeAdd/UnsafeSub/UnsafeMul/UnsafeDiv in vyperTypeSystemScript.sml:93-96. API corrupted /\ to ∧ but HOL4 accepts both. -> well_typed_binop_no_type_error SAVED (confirmed in build log line 7). The definition fix was the ROOT CAUSE of 4 unprovable msg≠'binop' goals. ()
  - Build vyperTypeBuiltinsTheory fails at extract_ec_point_uint256_2_not_none -> Pre-existing failure (not caused by our fix). Theorem needs ALOOKUP_sparse_has_type bridge to derive value_has_type (BaseTV (UintT 256)) x from ALOOKUP l = SOME x + sparse_has_type. ()
  - Attempted ALOOKUP_sparse_has_type_local[local] helper with several proof approaches: Induct_on al >> simp[value_has_type_def] >> Cases, Induct >> fs[ALOOKUP_DEF] >> Cases_on h -> Various failures: (1) sparse_has_type_def not exported from vyperTyping, (2) >- (fs[] >> metis_tac[]) causes HOL4 type error from precedence, (3) metis_tac can't unfold sparse_has_type. Latest untested attempt: Induct >> fs[ALOOKUP_DEF] with >- structure. ()
- `E0172` (progressed, tool_limit)
  - Added vyperAssignPreservesType ancestor to import ALOOKUP_sparse_has_type; removed local duplicate that used unavailable ALOOKUP_DEF -> ALOOKUP_sparse_has_type_local build error resolved; imported ALOOKUP_sparse_has_type from vyperAssignPreservesType ()
  - Generalized extract_ec_point_uint256_2_not_none from !av ... ArrayV av to !v ... v, then use metis_tac[helper] in ecadd caller instead of Cases_on (EL 0 vs) -> Generalized helper proof written; ecadd_no_type_error proof rewritten with Length+value_has_type+metis approach - NOT YET TESTED ()
  - Tried gvs[evaluate_ecadd_def, AllCaseEqs(), ...] to resolve ecadd after value/Cases_on decomposition -> FAILED: AllCaseEqs creates 1020+ goals from nested case splits. Use boundary lemmas + simp instead. ()
  - Tried fs[evaluate_ecadd_def, AllCaseEqs(), ...] as alternative to gvs -> FAILED: fs doesn't do variable elimination so NoneV/type mismatch branches persist as impossible goals ()
- `E0173` (progressed, missing_helper)
  - Proved ALOOKUP_sparse_uint256_is_IntV boundary lemma via imp_res_tac ALOOKUP_sparse_has_type >> fs[vht_BaseTV_UintT] -> SUCCESS: boundary lemma proved and builds (`TO_type_system_rewrite-20260516T153850Z_m24752_t001`)
  - Proved extract_ec_point_uint256_2_not_none via: vht_ArrayTV_exists >> Cases_on av >> fs[value_has_type_def] >> simp[extract_ec_point_def, array_index_def, default_value_def] >> Cases_on ALOOKUP >> simp[CaseEq option, CaseEq value, vht_BaseTV_UintT] >> derive IntV via ALOOKUP_sparse_uint256_is_IntV >> rw[] -> SUCCESS: theorem proved and builds. Key insight: avoid gvs on ≠ NONE goal (negates conclusion), use fs+simp instead, then CaseEq to normalize case expressions in goal (`TO_type_system_rewrite-20260516T153850Z_m24758_t001`)
  - ecadd_no_type_error: Cases_on vs >> gvs[LIST_REL_CONS1] >> Cases_on t >> gvs[LIST_REL_CONS1] >> derive extract_ec_point ≠ NONE via metis >> simp[evaluate_builtin_def, evaluate_ecadd_def] >> rpt (CASE_TAC >> simp[]) -> FAILED: metis_tac[extract_ec_point_uint256_2_not_none] can't derive type values (x/x') from MAP/EVERY/LIST_REL assumptions. Need to bridge from tvs to concrete ArrayTV type values. evaluate_type inversion for ArrayT is missing. (`TO_type_system_rewrite-20260516T153850Z_m24767_t001`)
- `E0174` (progressed, missing_helper)
  - Removed blanket gvs[evaluate_type_def] from well_typed_builtin_app_no_type_error dispatcher (line 1787). This caused irule calls to fail because gvs expanded evaluate_type for FlagT/BytesT into unresolved case expressions that didn't match helper lemma premises. -> Reduced 26 remaining goals to 25 - FlagT Not case now closes via irule flag_Not_no_type_error (it expects abstract evaluate_type assumption) ()
  - Added imp_res_tac evaluate_type_BaseT_SOME + gvs[evaluate_type_def, evaluate_builtin_def, ...] catch-all block for remaining cases, plus TRY (Cases_on `bd`) and TRY (Cases_on `d`) for abstract BytesT bd and denomination d -> Reduced from 25 to ~24 goals. Cases_on `d` closes one AsWeiValue case. Cases_on `bd` doesn't help Keccak256/Sha256 because after gvs the assumption is already a case-expression on bd, and the TRY fails on non-bd goals. ()
  - Replaced irule-based delegation blocks with direct expansion: imp_res_tac evaluate_type_BaseT_SOME >> gvs[evaluate_type_def, evaluate_builtin_def, ...] for remaining cases -> Partially works: evaluates most definitions. But abstract bd/d variables still block goal closure. Need per-category case splits that succeed across ALL goals (not TRY which fails on non-matching goals). ()
- `E0175` (progressed, missing_helper)
  - Verified AsWeiValue IntT unprovability with EVAL probe: evaluate_as_wei_value Kwei (IntV (-1)) = INR (TypeError ewv neg). Fixed definition: is_numeric_type -> is_uint_type for AsWeiValue in vyperTypeSystemScript.sml. After fix, 24->22 remaining goals. Key remaining: Keccak256/Sha256 need Cases_on bd before gvs evaluate_type_def; Concat/Slice fall through irule because gvs[evaluate_builtin_def] changes conclusion shape from evaluate_builtin ... to evaluate_concat ..., breaking irule match with helper lemmas. -> Definition fix confirmed. AsWeiValue IntT/DecimalT goals eliminated. 22 remaining goals identified in instrumented log. Root causes of remaining goals identified but not yet fixed. (`TO_type_system_rewrite-20260516T153850Z_m25010_t001`)
- `E0176` (progressed, bad_definition)
  - Analysis of 22 remaining goals in well_typed_builtin_app_no_type_error from instrumented log. Identified 4 categories: (1) Keccak256/Sha256 (2 goals): evaluate_type produces case expr on abstract bd; need Cases_on bd before gvs evaluate_type_def. (2) Concat/Slice (4 goals): gvs[evaluate_builtin_def] changes conclusion from evaluate_builtin... to evaluate_concat..., breaking irule match with concat_no_type_error/slice_no_type_error helpers. (3) Concat/Slice with StringT (2 goals): same issue. (4) Simple builtins (Ceil/Floor/Neg/AddMod/MulMod/PowMod256/BlockHash/BlobHash/MethodId/Uint2Str/MakeArray, ~14 goals): need Cases_on abstract variables + evaluate_type_def + evaluate_builtin_def expansion. -> Root cause identified: monolithic TRY block architecture is wrong. Different builtin categories need different proof strategies (Cases_on bd BEFORE vs AFTER gvs, irule BEFORE vs AFTER evaluate_builtin_def expansion). TRY blocks silently backtrack on non-matching goals, letting them fall to catch-all gvs[evaluate_builtin_def] which irreversibly changes conclusion shape. Fix: replace monolithic proof with per-builtin-category sub-theorems. ()
- `E0177` (stuck, bad_definition)

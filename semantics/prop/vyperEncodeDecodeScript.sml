(*
 * vyperEncodeDecodeScript.sml
 *
 * Storage encode/decode roundtrip properties.
 *
 * TOP-LEVEL:
 *   encode_decode_roundtrip_ok_def - predicate: type/value pair round-trips through storage
 *   encode_decode_roundtrip_all - all well-formed values round-trip
 *)
...
Definition encode_decode_roundtrip_ok_def:
  encode_decode_roundtrip_ok tv v ⇔
    ∀writes base storage.
...
Theorem word_of_bytes_replicate_zero[local]:
  ∀a n. word_of_bytes T a (REPLICATE n (0w:word8)) = (0w : bytes32)
Proof
  Induct_on `n` >> simp[byteTheory.word_of_bytes_def] >>
  simp[byteTheory.set_byte_def, LET_THM, byteTheory.word_slice_alt_zero]
QED

Theorem word_of_bytes_zero_pad[local]:
  ∀bs a n.
    word_of_bytes T a (bs ++ REPLICATE n (0w:word8)) =
    (word_of_bytes T a bs : bytes32)
Proof
  Induct >> simp[byteTheory.word_of_bytes_def, word_of_bytes_replicate_zero]
QED
...
Theorem decode_value_single_slot[local]:
  ∀bt v slot.
    ...
    encode_decode_roundtrip_ok (BaseTV bt) v
Proof
...
(* ===== encode_decode_roundtrip_ok: BoolT ===== *)
...
Theorem encode_decode_roundtrip_ok_bool[local]:
  ∀b. encode_decode_roundtrip_ok (BaseTV BoolT) (BoolV b)
Proof
...
(* ===== encode_decode_roundtrip_ok: AddressT ===== *)

Theorem encode_decode_roundtrip_ok_address[local]:
  ∀bs. LENGTH bs = 20 ⇒ encode_decode_roundtrip_ok (BaseTV AddressT) (BytesV (Fixed 20) bs)
Proof
  rw[] >> irule decode_value_single_slot >> simp[] >>
...
(* ===== encode_decode_roundtrip_ok: BytesT (Fixed n) ===== *)

Theorem encode_decode_roundtrip_ok_fixed_bytes[local]:
  ∀n bs. LENGTH bs = n ∧ n ≤ 32 ⇒
    encode_decode_roundtrip_ok (BaseTV (BytesT (Fixed n))) (BytesV (Fixed n) bs)
Proof
  rw[] >> irule decode_value_single_slot >> simp[] >>
  qexists_tac `word_of_bytes_be bs` >>
  simp[encode_base_to_slot_def, decode_base_from_slot_def] >>
  simp[word_of_bytes_be_def] >>
  `∀bs:word8 list.
     word_of_bytes T (0w:bytes32) bs =
     word_of_bytes T 0w (bs ++ REPLICATE (32 - LENGTH bs) (0w:word8))`
    by (gen_tac >> irule (GSYM word_of_bytes_zero_pad)) >>
  pop_assum (fn th =>
    CONV_TAC (LAND_CONV (RAND_CONV (ONCE_REWRITE_CONV [th])))) >>
  REWRITE_TAC[GSYM word_of_bytes_be_def] >>
  DEP_REWRITE_TAC[word_to_bytes_be_word_of_bytes_be] >>
...
Theorem w2n_i2w_num[local]:
  ∀i. 0 ≤ i ∧ Num i < dimword(:256) ⇒
    w2n ((i2w i) : bytes32) = Num i
Proof
  rpt strip_tac >>
  `&(w2n ((i2w i):bytes32)) = &(Num i) :int` suffices_by
    simp[integerTheory.INT_INJ] >>
  ONCE_REWRITE_TAC [integer_wordTheory.w2n_i2w] >>
  `&(Num i):int = i` by (
    REWRITE_TAC [integerTheory.INT_OF_NUM] >> simp[]) >>
  pop_assum SUBST_ALL_TAC >>
  irule integerTheory.INT_LESS_MOD >>
  conj_tac >- simp[] >>
  Cases_on `i` >> gvs[integerTheory.INT_OF_NUM]
QED

Theorem encode_decode_roundtrip_ok_uint[local]:
  ∀n i. 0 ≤ i ∧ i < &(2 ** n) ∧ n ≤ 256 ⇒
    encode_decode_roundtrip_ok (BaseTV (UintT n)) (IntV (Unsigned n) i)
Proof
  rw[] >> irule decode_value_single_slot >> simp[] >>
  qexists_tac `i2w i` >>
  simp[encode_base_to_slot_def, decode_base_from_slot_def] >>
  ...
  simp[w2n_i2w_num, integerTheory.INT_OF_NUM]
QED

(* Helper: sign-extension roundtrip for non-negative integers *)
Theorem sign_extend_roundtrip_nonneg[local]:
  ∀n (i:int). 0 ≤ i ∧ i < &(2 ** (n − 1)) ∧ n ≤ 256 ∧ 0 < n ⇒
    (let v = w2n ((i2w i):bytes32) MOD 2 ** n in
       if v < 2 ** (n − 1) then &v else &v − &(2 ** n)) = i
Proof
  rpt strip_tac >>
  `Num i < 2 ** (n − 1)` by (Cases_on `i` >> gvs[]) >>
  `Num i < 2 ** n` by (
    irule LESS_LESS_EQ_TRANS >> qexists_tac `2 ** (n − 1)` >> simp[]) >>
  `Num i < dimword(:256)` by (
    irule LESS_LESS_EQ_TRANS >> qexists_tac `2 ** n` >>
    simp[wordsTheory.dimword_def]) >>
  `w2n ((i2w i):bytes32) MOD 2 ** n = Num i` by
    simp[w2n_i2w_num] >>
  simp[integerTheory.INT_OF_NUM]
QED

(* Helper: w2n of i2w for negative values *)
Theorem w2n_i2w_neg[local]:
  ∀m. 0 < m ∧ m < dimword(:256) ⇒
    w2n ((i2w (-&m)):bytes32) = dimword(:256) − m
Proof
  rw[integer_wordTheory.i2w_def, integerTheory.INT_NEGNEG,
     integerTheory.NUM_OF_INT, wordsTheory.word_2comp_n2w,
     wordsTheory.w2n_n2w]
QED

(* Helper: 2^a divides 2^b when a ≤ b *)
Theorem twoexp_divides[local]:
  ∀a b. a ≤ b ⇒ (2 ** b − 2 ** a) MOD 2 ** a = 0
Proof
  rpt strip_tac >>
  `2 ** a ≤ 2 ** b` by simp[bitTheory.TWOEXP_MONO2] >>
  `∃k. 2n ** b − 2 ** a = k * 2 ** a` by (
    qexists_tac `2 ** (b − a) − 1` >>
    `2n ** b = 2 ** a * 2 ** (b − a)` by simp[GSYM EXP_ADD] >>
    gvs[] >>
    ONCE_REWRITE_TAC [RIGHT_SUB_DISTRIB] >>
    simp[MULT_COMM]) >>
  simp[]
QED

(* Helper: (dimword - m) MOD 2^n = 2^n - m *)
Theorem sub_mod_dimword[local]:
  ∀m n. 0 < m ∧ m ≤ 2 ** n ∧ n ≤ 256 ⇒
    (dimword(:256) − m) MOD 2 ** n = 2 ** n − m
Proof
  rpt strip_tac >>
  `2 ** n ≤ dimword(:256)` by (
    simp[wordsTheory.dimword_def] >>
    irule bitTheory.TWOEXP_MONO2 >> simp[]) >>
  (* Express dimword as (q+1) * 2^n for some q *)
  `∃q. dimword(:256) = (q + 1) * 2 ** n` by (
    `dimindex(:256) = 256` by EVAL_TAC >>
    qexists_tac `2 ** (256 − n) − 1` >>
    REWRITE_TAC [wordsTheory.dimword_def] >>
    pop_assum SUBST1_TAC >>
    simp[SUB_ADD, GSYM EXP_ADD]) >>
  (* Now dimword - m = (q+1) * 2^n - m, use MOD_TIMES_SUB *)
  simp[MOD_TIMES_SUB]
QED

(* Helper: m < dimword(:256) from m ≤ 2^(n-1) and n ≤ 256 *)
Theorem le_exp_lt_dimword[local]:
  ∀m k. m ≤ 2 ** k ∧ k < 256 ⇒ m < dimword(:256)
Proof
  rpt strip_tac >>
  `2 ** k < dimword(:256)` by (
    `dimindex(:256) = 256` by EVAL_TAC >>
...
(* Helper: for negative i, w2n (i2w i) MOD 2^n = Num(i + &(2^n)) *)
Theorem w2n_i2w_mod_neg[local]:
  ∀n (i:int). i < 0 ∧ -(&(2 ** (n − 1))) ≤ i ∧ n ≤ 256 ∧ 0 < n ⇒
    w2n ((i2w i):bytes32) MOD 2 ** n = Num (i + &(2 ** n))
Proof
  rpt strip_tac >>
  Cases_on `i` >> gvs[] >>
  rename1 `m ≤ 2 ** (n − 1)` >>
  `m ≤ 2 ** n` by (
    irule LESS_EQ_TRANS >> qexists_tac `2 ** (n − 1)` >> simp[]) >>
  (* Simplify RHS *)
  `(-&m + &(2 ** n)):int = &(2 ** n − m)` by (
    ONCE_REWRITE_TAC [integerTheory.INT_ADD_COMM] >>
    REWRITE_TAC [GSYM integerTheory.int_sub] >>
    simp[integerTheory.INT_SUB]) >>
  simp[integerTheory.NUM_OF_INT] >>
  (* Simplify LHS *)
  `m < dimword(:256)` by (irule le_exp_lt_dimword >> qexists_tac `n − 1` >> simp[]) >>
  `w2n ((i2w (-&m)):bytes32) = dimword(:256) − m` by simp[w2n_i2w_neg] >>
  pop_assum (fn th => REWRITE_TAC [th]) >>
  irule sub_mod_dimword >> simp[]
QED

(* Helper: sign-extension roundtrip for negative integers *)
Theorem sign_extend_roundtrip_neg[local]:
  ∀n (i:int). i < 0 ∧ -(&(2 ** (n − 1))) ≤ i ∧ n ≤ 256 ∧ 0 < n ⇒
    (let v = w2n ((i2w i):bytes32) MOD 2 ** n in
       if v < 2 ** (n − 1) then &v else &v − &(2 ** n)) = i
Proof
  rpt strip_tac >>
  `w2n ((i2w i):bytes32) MOD 2 ** n = Num (i + &(2 ** n))` by
    (irule w2n_i2w_mod_neg >> simp[]) >>
  simp[] >>
  `0 ≤ i + &(2 ** n)` by (
    Cases_on `i` >> gvs[] >>
    rename1 `m ≤ 2 ** (n − 1)` >>
  ...
  `Num (i + &(2 ** n)) ≥ 2 ** (n − 1)` by (
    ...
    `m ≤ 2 ** n` by (
      irule LESS_EQ_TRANS >> qexists_tac `2 ** (n − 1)` >> simp[]) >>
    simp[integerTheory.INT_ADD_CALCULATE, integerTheory.NUM_OF_INT] >>
    `n = SUC (n − 1)` by simp[] >>
    pop_assum (fn th => REWRITE_TAC [Once th, EXP]) >>
    DECIDE_TAC) >>
  simp[] >>
  Cases_on `i` >> gvs[] >>
  rename1 `m ≤ 2 ** (n − 1)` >>
  ...
  `Num (-&m + &(2 ** n)) = 2 ** n − m` by
    simp[integerTheory.INT_ADD_CALCULATE, integerTheory.NUM_OF_INT] >>
  simp[GSYM integerTheory.INT_SUB, integerTheory.int_sub] >>
  ONCE_REWRITE_TAC [integerTheory.INT_ADD_COMM] >>
  simp[integerTheory.INT_ADD_ASSOC, integerTheory.INT_ADD_RINV]
QED

(* ===== encode_decode_roundtrip_ok: IntT ===== *)

Theorem encode_decode_roundtrip_ok_int[local]:
  ∀n i. -(&(2 ** (n − 1))) ≤ i ∧ i < &(2 ** (n − 1)) ∧ n ≤ 256 ∧ 0 < n ⇒
    encode_decode_roundtrip_ok (BaseTV (IntT n)) (IntV (Signed n) i)
Proof
  rw[] >> irule decode_value_single_slot >> simp[] >>
  qexists_tac `i2w i` >>
  simp[encode_base_to_slot_def, decode_base_from_slot_def] >>
  Cases_on `0 ≤ i`
  >- (drule_all sign_extend_roundtrip_nonneg >> simp[])
  >>
  `i < 0` by gvs[integerTheory.INT_NOT_LE] >>
  drule_all sign_extend_roundtrip_neg >> simp[]
QED
...
Proof
  rpt strip_tac >> irule decode_value_single_slot >> simp[] >>
  qexists_tac `i2w i` >>
  REWRITE_TAC[encode_base_to_slot_def, decode_base_from_slot_def] >>
  `(let v = w2n ((i2w i):bytes32) MOD 2 ** 168 in
     if v < 2 ** 167 then &v else &v − &(2 ** 168)) = i` suffices_by simp[] >>
  `167 = 168 − 1n` by simp[] >>
  pop_assum (fn th => REWRITE_TAC [th]) >>
  Cases_on `0 ≤ i`
  >- (irule sign_extend_roundtrip_nonneg >> simp[] >> gvs[])
  >>
  `i < 0` by gvs[integerTheory.INT_NOT_LE] >>
  irule sign_extend_roundtrip_neg >> simp[] >> gvs[]
QED
...
Theorem encode_decode_roundtrip_ok_flag[local]:
  ∀m k. k < dimword(:256) ⇒
    encode_decode_roundtrip_ok (FlagTV m) (FlagV m k)
Proof
  rw[encode_decode_roundtrip_ok_def] >>
  gvs[encode_value_def, AllCaseEqs()] >>
  gvs[decode_value_def, read_slot_after_apply_writes_single,
...
Theorem encode_decode_roundtrip_ok_none[local]:
  encode_decode_roundtrip_ok NoneTV NoneV
Proof
  rw[encode_decode_roundtrip_ok_def, encode_value_def, decode_value_def, apply_writes_nil]
QED
...
(* Bounds compatibility: value bounds match type bounds (recursive).
...
Definition bounds_compat_def:
  ...
  bounds_compat (ArrayTV tv (Fixed n)) (ArrayV (SArrayV _ _ sparse)) =
    EVERY (λ(k,v). bounds_compat tv v) sparse ∧
  bounds_compat (ArrayTV tv (Dynamic _)) (ArrayV (DynArrayV _ _ vs)) =
    EVERY (bounds_compat tv) vs ∧
  bounds_compat (StructTV ftypes) (StructV fields) =
...
(* encode_decode_roundtrip_ok for dynamic bytes *)
Theorem encode_decode_roundtrip_ok_dyn_bytes[local]:
  ∀max bs.
    max < dimword(:256) ∧ LENGTH bs ≤ max ⇒
    encode_decode_roundtrip_ok (BaseTV (BytesT (Dynamic max))) (BytesV (Dynamic max) bs)
Proof
  rw[encode_decode_roundtrip_ok_def] >>
...
(* encode_decode_roundtrip_ok for strings *)
Theorem encode_decode_roundtrip_ok_string[local]:
  ∀max s.
    max < dimword(:256) ∧ LENGTH s ≤ max ⇒
    encode_decode_roundtrip_ok (BaseTV (StringT max)) (StringV max s)
Proof
  rw[encode_decode_roundtrip_ok_def] >>
  gvs[encode_value_def, AllCaseEqs(), encode_dyn_bytes_slots_def] >>
  simp[Once decode_value_def] >>
...
(* ===== General encode_decode_roundtrip_ok from well_formed_value ===== *)

(* For non-compound types, encode_decode_roundtrip_ok follows from well_formed_value
...
Proof
  ...
  FIRST [
    ...
    irule encode_decode_roundtrip_ok_fixed_bytes >> simp[],
    irule encode_decode_roundtrip_ok_dyn_bytes >> simp[],
    irule encode_decode_roundtrip_ok_address >> simp[],
    irule encode_decode_roundtrip_ok_flag >> simp[]
  ] >> gvs[]
QED
...
(* For non-compound types (BaseTV, FlagTV, NoneTV), encode_decode_roundtrip_ok follows
...
Theorem encode_decode_roundtrip_ok_noncompound[local]:
...
Proof
  ...
  >- (Cases_on `encode_value tv v` >> gvs[encode_decode_roundtrip_ok_def]) >>
  Cases_on `tv` >> fs[]
  (* BaseTV *)
  >- (
    irule encode_decode_roundtrip_ok_from_well_formed_base >> simp[] >>
...
Proof
  ...
  conj_tac >- (
    REWRITE_TAC [wordsTheory.w2n_n2w] >>
    irule LESS_EQ_TRANS >>
    qexists_tac `off2 + type_slot_size tv` >>
    reverse conj_tac >- fs[] >>
    simp[arithmeticTheory.MOD_LESS_EQ]) >>
  qexists_tac `sz` >>
  REWRITE_TAC [wordsTheory.w2n_n2w] >>
  Cases_on `off2 < dimword(:256)` >- fs[LESS_MOD] >>
  `off2 = dimword(:256) ∧ type_slot_size tv = 0` by fs[] >>
  gvs[DIVMOD_ID]
QED

(* Helper: encode_decode_roundtrip_ok at an offset.
   If tv v has encode_decode_roundtrip_ok, then encoding v and applying the shifted writes
   at offset k allows decode at base + k to recover v. *)
Theorem encode_decode_roundtrip_ok_at_offset[local]:
...
Theorem encode_decode_roundtrip_at_offset_disjoint[local]:
...
Proof
  ...
  >> irule encode_decode_roundtrip_ok_at_offset >> simp[]
QED
...
Theorem encode_decode_tuple[local]:
  ∀tvs offset vs writes base storage.
    EVERY (λtv. ∀v. well_formed_value v ∧ bounds_compat tv v ⇒
           encode_decode_roundtrip_ok tv v) tvs ∧
    LIST_REL bounds_compat tvs vs ∧
...
Proof
  ...
  `decode_value (apply_writes (n2w base) writes storage)
    ...
    irule encode_decode_roundtrip_at_offset_disjoint >> simp[] >>
    conj_tac >- (
      rpt strip_tac >>
...
Theorem encode_decode_dyn_array[local]:
  ∀vs offset tv writes base storage.
    (∀v. well_formed_value v ∧ bounds_compat tv v ⇒
         encode_decode_roundtrip_ok tv v) ∧
    EVERY (bounds_compat tv) vs ∧
...
Theorem encode_decode_struct[local]:
  ∀ftypes offset fields writes base storage.
    EVERY (λ(nm,tv). ∀v. well_formed_value v ∧ bounds_compat tv v ⇒
             encode_decode_roundtrip_ok tv v) ftypes ∧
    LIST_REL (λ(_, tv) (_, v). bounds_compat tv v) ftypes fields ∧
...
Proof
  ...
  Cases_on `h` >>
  gvs[LIST_REL_CONS1, FORALL_PROD] >>
...
Proof
  ...
  >- (
    gvs[] >>
    irule encode_decode_roundtrip_at_offset_disjoint >>
    simp[] >>
...
Theorem encode_decode_roundtrip_static_array[local]:
...
Theorem encode_decode_roundtrip_dyn_array[local]:
...
Theorem encode_decode_roundtrip_all:
  ∀tv v.
    well_formed_value v ∧ well_formed_type_value tv ∧ bounds_compat tv v ⇒
    encode_decode_roundtrip_ok tv v
Proof
  ...
  >> TRY (
    irule encode_decode_roundtrip_ok_noncompound >>
    simp[well_formed_value_def, well_formed_type_value_def] >>
    gvs[bounds_compat_def] >> NO_TAC
  )
  (* TupleTV tvs *)
  >> TRY (
    rename1 `TupleTV tvs` >>
    (* Bridge IH: bounds_compat + well_formed → roundtrip *)
    `EVERY (λtv. ∀v'. well_formed_value v' ∧ bounds_compat tv v' ⇒
           encode_decode_roundtrip_ok tv v') tvs` by (
    ...
    (* Expand encode_decode_roundtrip_ok in goal only *)
    CONV_TAC (REWR_CONV encode_decode_roundtrip_ok_def) >>
    rpt strip_tac >>
    ...
    gvs[well_formed_value_def, well_formed_type_value_def, type_slot_size_def,
        bounds_compat_def] >>
    RULE_ASSUM_TAC (CONV_RULE (TRY_CONV (DEPTH_CONV ETA_CONV))) >>
    gvs[] >> NO_TAC
  )
  (* ArrayTV tv' b *)
  >- (
    ...
    >- (
      ...
      (* n > 0: use encode_decode_roundtrip_static_array *)
      irule encode_decode_roundtrip_static_array >>
      gvs[well_formed_value_def, well_formed_type_value_def,
          type_slot_size_def, bounds_compat_def] >>
      rpt strip_tac >>
    ...
    >> (
      ...
      irule encode_decode_roundtrip_dyn_array >> simp[] >>
      conj_tac >- (
        rpt strip_tac >>
      ...
      gvs[well_formed_type_value_def, type_slot_size_def, bounds_compat_def] >>
      RULE_ASSUM_TAC (CONV_RULE (TRY_CONV (DEPTH_CONV ETA_CONV))) >>
      gvs[]
    )
  )
  ...
  >> (
    ...
    (* Bridge IH: bounds_compat + well_formed → roundtrip *)
    `EVERY (λ(nm,tv). ∀v'. well_formed_value v' ∧ bounds_compat tv v' ⇒
             encode_decode_roundtrip_ok tv v') ftypes` by (

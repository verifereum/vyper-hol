...
Definition eval_base_target_cps_def:
  ...
  eval_base_target_cps cx (ImmutableNameTarget id) st k =
    (let r = do
...
Definition eval_expr_cps_def:
  ...
  eval_expr_cps cx1 (ImmutableName id) st k =
    liftk cx1 ApplyTv
...
Definition apply_tv_def:
  ...
  apply_tv cx tv st (ArrayK k) =
    liftk cx ApplyVal (materialise cx tv st) (ArrayK k) ∧
...
Definition apply_val_def:
  ...
  apply_val cx v st (ArrayK k) =
    liftk cx ApplyVals
      (lift_option_type (extract_elements v) "For not ArrayV" st) k ∧
...
Proof
  ...
  \\ conj_tac >- ( (* eval_stmts (s::ss) *)
    ...
    \\ gvs[]
    \\ first_x_assum drule \\ rw[])
  \\ conj_tac >- ( (* eval_iterator (Array e) - changed: materialise *)
  ...
  \\ conj_tac >- rw[eval_base_target_cps_def, evaluate_def, liftk1] (* eval_base_target (ImmutableNameTarget id) *)
  ...
  \\ conj_tac >- rw[eval_expr_cps_def, evaluate_def, liftk1] (* eval_expr (ImmutableName id) *)
  ...
  \\ conj_tac >- ( (* eval_expr (StructLit ...) *)
    ...
    \\ simp[apply_tv_def]
    \\ rw[Once OWHILE_THM, stepk_def] )
  \\ conj_tac >- ( (* eval_expr (Subscript e1 e2) - changed *)
    rw[eval_expr_cps_def, evaluate_def, bind_def]
    \\ CASE_TAC \\ gvs[cont_def] \\ reverse CASE_TAC
    >- rw[Once OWHILE_THM, stepk_def, apply_exc_def]
  ...
  \\ conj_tac >- ( (* ExtCall *)
    ...
    \\ pairarg_tac \\ gvs[bind_def,CaseEq"prod",CaseEq"sum"]
    \\ pairarg_tac \\ gvs[bind_def,CaseEq"prod",CaseEq"sum"]
  ...
  (* eval_exprs (e::es) - changed: materialise *)
  \\ rw[eval_expr_cps_def, evaluate_def, bind_def]
  \\ CASE_TAC \\ gvs[cont_def] \\ reverse CASE_TAC
  >- rw[Once OWHILE_THM, stepk_def, apply_exc_def]
  >> rw[Once OWHILE_THM, stepk_def, apply_tv_def, liftk1]

...
Datatype:
  ...
  array_value
  = DynArrayV num (value list)
  | SArrayV num ((num, value) alist)
  | TupleV (value list)
End
...
Definition default_value_def:
  ...
  default_value (ArrayTV t (Dynamic n)) = ArrayV (DynArrayV n []) ∧
  default_value (ArrayTV t (Fixed n)) = ArrayV (SArrayV n []) ∧
...
Definition make_array_value_def:
  make_array_value tv (Fixed n) vs =
    SArrayV n (enumerate_static_array (default_value tv) 0 vs) ∧

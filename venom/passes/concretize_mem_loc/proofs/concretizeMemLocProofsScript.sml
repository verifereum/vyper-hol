 ...
 * concrete offset, and GEP with ADD. The original program
 ...
 *   - pointer-derived vars (GEP outputs, etc.): hold derived addresses
 ...
 * like MSTORE/MLOAD/MCOPY) or passed through GEP/ADD. They never
...
(* Variables whose values may differ due to memory remapping.
   ...
   transitively derived from them (GEP outputs, ADD of pointer
...
(* Pointer-derived variables are only used as memory addresses.
   ...
   - pointer arithmetic (GEP, ADD) whose output is also pointer-derived
   ...
   memory offsets used only for MSTORE/MLOAD/MCOPY via GEP. *)
Definition pointer_confined_def:
  pointer_confined fn (amap : alloc_map) <=>
    ...
    !bb inst v.
      ...
      (inst.inst_opcode = GEP \/ inst.inst_opcode = ADD) /\
...
(* Preconditions:
   ...
   - GEP→ADD: identical semantics (exec_pure2 word_add). Output is

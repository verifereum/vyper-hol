...
Theorem result_equiv_is_lift_result[local]:
  !vars r1 r2.
    ...
    lift_result (state_equiv vars) (execution_equiv vars) r1 r2
Proof
...
Theorem remove_unused_block_sim[local]:
  !n fuel ctx lr fence V bb s1 s2.
    ...
    lift_result (state_equiv V) (execution_equiv V)
      (run_block fuel ctx bb s1)
      (run_block fuel ctx (remove_unused_block lr fence bb) s2)
Proof
  completeInduct_on `n` >>
  rpt strip_tac >> gvs[] >>
...
Theorem lift_result_compose_result_equiv[local]:
  !vars r1 r2 r3.
    lift_result (state_equiv vars) (execution_equiv vars) r1 r2 /\
    result_equiv {} r2 r3 ==>
    lift_result (state_equiv vars) (execution_equiv vars) r1 r3
Proof
  rpt strip_tac >>
...
Theorem result_equiv_weaken_lift_result[local]:
  !V V_out r1 r2.
    ...
    (?e. r1 = Error e) \/
    lift_result (state_equiv V_out) (execution_equiv V_out) r1 r2
Proof
...
Theorem remove_unused_block_sim_ssa:
  !n fuel ctx fn bb s1 s2 V_in V.
    ...
    (?e. run_block fuel ctx bb s1 = Error e) \/
    lift_result (state_equiv V_out) (execution_equiv V_out)
      (run_block fuel ctx bb s1)
      (run_block fuel ctx (remove_unused_block lr fence bb) s2)
Proof
  completeInduct_on `n` >>
  ...
  >| [
    ...
    Cases_on `is_terminator (EL s2.vs_inst_idx bb.bb_instructions).inst_opcode`
    >- (
      (* Terminator kept OK-OK: run_block returns Halt or OK based on vs_halted *)
      gvs[] >>
...
Proof
  ...
  (* Induction on fuel *)
  Induct_on `fuel`
  >- (rw[] >> DISJ1_TAC >> simp[venomExecSemanticsTheory.run_function_def])
  >>
  rw[] >>
  `s1.vs_current_bb = s2.vs_current_bb` by metis_tac[] >>
  `s2.vs_inst_idx = 0` by metis_tac[] >>
  ONCE_REWRITE_TAC[venomExecSemanticsTheory.run_function_def] >>
...
Theorem lift_result_refl_state_equiv:
  !vars r.
    lift_result (state_equiv vars) (execution_equiv vars) r r
Proof
...
Theorem lift_result_error_right[local]:
  !R_ok R_term r e.
    lift_result R_ok R_term r (Error e) ==> ?e'. r = Error e'
Proof
...
Theorem owhile_lift_result_compose:
  !G f R_ok R_term Q P.
    ...
    (!x. G x /\ P x ==>
      (?e. Q x = Error e) \/
      lift_result R_ok R_term (Q x) (Q (f x))) /\
    (!x. G x /\ P x ==> P (f x)) ==>
    !x y. OWHILE G f x = SOME y ==> P x ==>
      (?e. Q x = Error e) \/
      lift_result R_ok R_term (Q x) (Q y)
Proof
  rpt gen_tac >> strip_tac >>
  ...
  >> (* Step case: G x, IH for f x, need to compose *)
    ...
    strip_tac >>
    `(∃e. Q x = Error e) ∨ lift_result R_ok R_term (Q x) (Q (f x))` by metis_tac[] >>
    `P (f x)` by metis_tac[] >>
    fs[]
...
Proof
  ...
  mp_tac (BETA_RULE (Q.ISPECL [
    ...
    `\fn_i. run_function fuel ctx fn_i s`
...
Theorem pass_correct_empty_false_for_nop[local]:
  ~pass_correct (state_equiv {}) (execution_equiv {})
    (\fuel. run_function fuel ARB cx_ru_fn1
       (ARB with <|vs_vars := FEMPTY;
                   vs_current_bb := "entry"; vs_inst_idx := 0; vs_halted := F|>))
    (\fuel. run_function fuel ARB cx_ru_fn2
       (ARB with <|vs_vars := FEMPTY;
                   vs_current_bb := "entry"; vs_inst_idx := 0; vs_halted := F|>))
Proof

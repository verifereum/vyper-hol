(*
 * Remove Unused Variables — Correctness (Context-Level)
 *
 * pass_correct (state_equiv {}) (execution_equiv {}) is FALSE under wf_ssa
 * — machine-checked counterexample in removeUnusedProofsScript.sml.
 * Tightest correct relation:
 * pass_correct (state_equiv elim) (execution_equiv elim).
 *
 * We use all_eliminated (union over ALL functions) rather than per-function
 * eliminated vars because callee Halt/Abort propagates directly and may
 * carry the callee's eliminated vars in the halted state.
 *)
...
Triviality execution_equiv_mono:
...
Triviality elim_subset_all_elim:
...
Triviality fn_inst_wf_nop_outputs_empty:
...
Triviality ru_function_fn_name:
...
Triviality ru_function_entry_label:
...
Triviality lookup_function_ru_context:
  !name ctx.
...
Triviality lookup_function_ru_none:
  !name ctx.
...
Triviality merge_callee_execution_equiv:
  !V s1 s2 caller.
...
Triviality run_block_cross_ctx_change_ru:
  !n fuel ctx bb s.
 ...
 *)
Triviality remove_unused_cross_ctx_fn_equiv:
  !fuel ctx.
    venom_wf ctx /\

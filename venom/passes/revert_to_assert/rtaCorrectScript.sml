(*
 * Revert-to-Assert Block Execution Helpers
 *
 * This theory provides block-level execution helpers for the revert-to-assert pass.
 * These are used by rtaPattern1 to prove pattern execution correctness.
 *
 * TOP-LEVEL THEOREMS:
 *   - run_block_iszero_assert_reverts : Pattern 1 nonzero case (reverts)
 *   - run_block_iszero_assert_jumps   : Pattern 1 zero case (jumps)
 *   - revert_states_equiv_except      : Revert state equivalence
 *   - jumped_states_equiv_except      : Jumped state equivalence
 *)

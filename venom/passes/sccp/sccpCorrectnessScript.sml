(*
 * SCCP — Context-level Correctness
 *
 * Strategy: prove full execution equality
 *   ∀fuel. run_function fuel ctx f s = run_function fuel ctx' f' s
 * by fuel induction over all functions simultaneously.
 *
 * At each step:
 * - Non-INVOKE: step_inst_ctx_irrel + sccp_inst_step_correct_cond → equality
...
(* ================================================================= *)
(*  Part 1: Context-level infrastructure                             *)
(* ================================================================= *)

(* step_inst_ctx_irrel: now in venomExecPropsTheory *)

Triviality sccp_function_fn_name:
...
Triviality lookup_function_sccp_context:
  !entry fns f.
...
Triviality fn_dominates_dom_reachable:
...
Triviality fn_dominates_predecessor:
  !fn d n p.
...
Triviality fn_cfg_edge_in_dfs_pre:
  !f x y.
    wf_function f /\
    fn_cfg_edge f x y /\
...
Triviality fn_reachable_in_dfs_pre:
  !f lbl.
...
(* Helper: lookup_var preservation implies FDOM preservation *)
Triviality lookup_var_fdom:
  !x s s'.
...
(* FDOM only grows through run_block — no SSA needed *)
Triviality run_block_fdom_mono:
  !fuel ctx bb s s'.
...
Triviality run_block_output_in_fdom_aux:
...
Triviality run_block_outputs_in_fdom:
...
Triviality strict_dom_vars_defined_entry:
  !f s.
...
(* Preservation across block boundary *)
Triviality terminator_no_outputs:
...
(* Label uniqueness: two blocks with the same label must be the same *)
Triviality same_label_same_block:
  !f bb1 bb2.
...
Triviality strict_dom_vars_defined_preserved:
  !f bb s s' fuel ctx.
...
Triviality run_block_result_equiv_ctx:
...
Triviality sccp_block_transform_label:
...
Triviality sccp_fn_entry_label:
...
Triviality sccp_fn_blocks_nonempty:
...
Triviality setup_callee_sccp_fn:
  !f args s.
    ~NULL f.fn_blocks ==>
...
Triviality lookup_block_sccp_fn_none:
  !lbl f.
...
Triviality lookup_block_sccp_fn_some:
  !lbl f bb.
...
Triviality lookup_function_none_sccp_context:
  !name ctx.
...
Triviality run_block_ctx_change:
  !fuel ctx bb s.
    (!f callee_s.
      MEM f ctx.ctx_functions /\
      callee_s.vs_inst_idx = 0 /\
...
Triviality sccp_inst_is_terminator:
...
Triviality nophi_derive_cond_const_sound:
  !f bb idx lat s.
    bb_well_formed bb /\
...
Triviality sccp_run_block_eq_entry:
  !fuel ctx f bb s.
    wf_function f /\ wf_ssa f /\ fn_inst_wf f /\
    MEM bb f.fn_blocks /\ bb.bb_label = s.vs_current_bb /\
    MEM bb.bb_label (cfg_analyze f).cfg_dfs_pre /\
...
Triviality callee_entry_preconditions:
  !f s.
...
Triviality sccp_block_result_equiv_some:
  !fuel ctx f bb s.
    MEM bb f.fn_blocks /\
    bb.bb_label = s.vs_current_bb /\
    s.vs_inst_idx = 0 /\
...
Triviality sccp_run_function_equiv:
  !fuel ctx.
    venom_wf ctx /\
...
(* ================================================================= *)
(*  Part 11: Context-level pass_correct                              *)
(* ================================================================= *)

Triviality nophi_inv_entry:
  !f s.
    fn_entry_label f = SOME s.vs_current_bb /\
    FDOM s.vs_vars = {} ==>
    nophi_inv f s
...
(* Helper: entry_block NONE propagates through sccp_fn *)
Triviality entry_block_sccp_fn_none:
...
Triviality entry_block_sccp_fn_some:
  !fn entry_bb.

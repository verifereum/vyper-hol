 ...
 *   step_inst_base_output_determined — outputs agree when inputs agree
 *)
...
(* Output determinism (local action): if step_inst_base returns OK on two
   states that agree on all fields the instruction reads, the results agree
   on output variables and all tracked written state fields.
   This is the key lemma for commutativity of independent instructions. *)
Theorem step_inst_base_output_determined:
  !inst s1 s2 v1 v2.
    ...
    (* Only require field agreement for fields the instruction accesses.
       In the commute proof, effects_independent guarantees the other
       instruction preserves all fields this instruction accesses. *)
    (Eff_MEMORY IN (read_effects inst.inst_opcode UNION
                    write_effects inst.inst_opcode) ==>
       s1.vs_memory = s2.vs_memory) /\
    (Eff_TRANSIENT IN (read_effects inst.inst_opcode UNION
                       write_effects inst.inst_opcode) ==>
       s1.vs_transient = s2.vs_transient) /\
    ((Eff_STORAGE IN (read_effects inst.inst_opcode UNION
                      write_effects inst.inst_opcode) \/
      Eff_BALANCE IN (read_effects inst.inst_opcode UNION
                      write_effects inst.inst_opcode) \/
      Eff_EXTCODE IN (read_effects inst.inst_opcode UNION
                      write_effects inst.inst_opcode)) ==>
       s1.vs_accounts = s2.vs_accounts) /\
    (Eff_IMMUTABLES IN (read_effects inst.inst_opcode UNION
                        write_effects inst.inst_opcode) ==>
       s1.vs_immutables = s2.vs_immutables) /\
    (Eff_RETURNDATA IN (read_effects inst.inst_opcode UNION
                        write_effects inst.inst_opcode) ==>
       s1.vs_returndata = s2.vs_returndata) /\
    (Eff_LOG IN (read_effects inst.inst_opcode UNION
                 write_effects inst.inst_opcode) ==>

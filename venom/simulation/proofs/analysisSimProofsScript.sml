...
Ancestors
  ...
  venomState venomWf cfgAnalysisProps
Libs
...
Proof
  ...
  qsuff_tac
    `!fuel run_ctx s1 s2.
       ...
       lift_result R_ok R_term (run_blocks fuel run_ctx fn s1)
  ...
  `lift_result R_ok R_term (exec_block fuel run_ctx bb s1)
  ...
  `(?e. exec_block fuel run_ctx bb s2 = Error e) \/
   lift_result R_ok R_term (exec_block fuel run_ctx bb s2)
...
Proof
  ...
  `lift_result R_ok R_term (exec_block fuel run_ctx bb s1)
    ...
    impl_tac >- (rpt conj_tac >> first_assum ACCEPT_TAC) >>
    disch_then drule_all >> simp[]) >>
  (* Per-block sim using analysis_block_sim_inv *)
  `(!idx. SUC idx <= LENGTH bb.bb_instructions ==>
      df_at bottom result bb.bb_label (SUC idx) =
      transfer ctx (EL idx bb.bb_instructions)
        (df_at bottom result bb.bb_label idx))` by
  ...
  `(?e. exec_block fuel run_ctx bb s2 = Error e) \/
   lift_result R_ok R_term (exec_block fuel run_ctx bb s2)
    ...
    impl_tac >- (
      ...
      (* state_inv preservation by step_inst *)
...
Proof
  ...
  `lift_result R_ok R_term (exec_block fuel run_ctx bb s1)
    ...
    impl_tac >- (rpt conj_tac >> first_assum ACCEPT_TAC) >>
    disch_then drule_all >> simp[]) >>
  (* Per-block sim using analysis_block_sim_inv_block *)
  `(!idx. SUC idx <= LENGTH bb.bb_instructions ==>
      df_at bottom result bb.bb_label (SUC idx) =
      transfer ctx (EL idx bb.bb_instructions)
        (df_at bottom result bb.bb_label idx))` by
  ...
  `(?e. exec_block fuel run_ctx bb s2 = Error e) \/
   lift_result R_ok R_term (exec_block fuel run_ctx bb s2)
    ...
    mp_tac (Q.SPECL [`R_ok`, `R_term`, `sound`, `state_inv`, `f`, `bb`,
      ...
      analysisSimProofsBaseTheory.analysis_block_sim_inv_block) >>
    impl_tac >- (
      rpt conj_tac >>
      TRY (first_assum ACCEPT_TAC) >>
      TRY (fs[EVERY_MEM, venomWfTheory.fn_inst_wf_def] >>
   ...
   (f : 'a -> instruction -> instruction list).
    ...
    let result = df_analyze_widen Forward bottom join widen threshold
      ...
      (!bb fuel run_ctx s.
         ...
         lift_result R_ok R_term (exec_block fuel run_ctx bb s)
...
Proof
  ...
  \\ `!b. (btp b).bb_label = b.bb_label` by
       simp[Abbr `btp`, abtp_label]
  \\ `lift_result R_ok R_term (exec_block fuel run_ctx (bt bb) s2)
...
(* General forward analysis successor entry soundness.
   ...
   H2: sound respects state_equiv {}
   H3: transfer preserves SOME (transfer ctx inst v <> NONE when v <> NONE)
   H4: join-right soundness (sound b s /\ b <> NONE ==> sound (join a b) s)
   H5: edge_transfer preserves soundness
   H6: joined_sound: one predecessor edge-transferred boundary sound + non-NONE
       implies the full df_joined_val is sound.
       (Typically proved from H4 + H5 + entry value trivial soundness.)
*)
Theorem wf_function_bb_well_formed[local]:
  wf_function fn /\ MEM bb fn.fn_blocks ==> bb_well_formed bb
Proof
  rpt strip_tac >> fs[venomWfTheory.wf_function_def]
QED

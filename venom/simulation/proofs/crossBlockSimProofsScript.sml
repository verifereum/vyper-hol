      ...
      `run_blocks (SUC f + k) ctx fn s =
       ...
       | SOME bb' =>
        ...
        by (`SUC f + k = SUC (f + k)` by simp[] >>
            pop_assum SUBST1_TAC >> simp[Once run_blocks_def]) >>
      gvs[] >>
      Cases_on `exec_block f ctx bb s` >> gvs[] >>
      rename1 `_ = OK v` >>
...
Triviality run_blocks_ok_block_at:
  !ctx fn bb s v fuel.
    ...
    exec_block fuel ctx bb s = OK v /\ ~v.vs_halted ==>
...
Triviality run_blocks_at_non_ok_block:
  !f ctx fn bb s r.
    lookup_block s.vs_current_bb fn.fn_blocks = SOME bb /\
    exec_block f ctx bb s = r /\ (!v. r <> OK v) ==>
...
Triviality run_blocks_at_ok_halted_block:
  !f ctx fn bb s v.
    lookup_block s.vs_current_bb fn.fn_blocks = SOME bb /\
    exec_block f ctx bb s = OK v /\ v.vs_halted ==>
    ...
    ==>
      ...
      ==>
        ?fuel. terminates (run_blocks fuel ctx fn' v2) /\
          ((!v. r1 <> OK v) ==>
            lift_result R_ok R_term r1 (run_blocks fuel ctx fn' v2)) /\
          (!v. r1 = OK v ==>
            lift_result R_ok R_term (Halt v) (run_blocks fuel ctx fn' v2))
Proof
    ...
    ==>
      ...
      ==>
        ?fuel. terminates (run_blocks fuel ctx fn v1) /\
          ((!v. r2 <> OK v) ==>
            lift_result R_ok R_term (run_blocks fuel ctx fn v1) r2) /\
          (!v. r2 = OK v ==>
            lift_result R_ok R_term (run_blocks fuel ctx fn v1) (Halt v))
Proof
  rpt gen_tac >> strip_tac >>
  Induct_on `n` >> rpt strip_tac
  >- (gvs[resolves_to_def] >>
...
Triviality fuel_alignment:
  ...
  ==>
    lift_result R_ok R_term (run_blocks f1 ctx fn s1)
...
Triviality resolve_left_lift:
  ...
  ==>
    lift_result R_ok R_term (run_blocks f ctx fn v)
...
Triviality left_terminal_right_fn:
  ...
  ==>
    ?fuel'. terminates (run_blocks fuel' ctx fn' s2) /\
      ((!v. r1 <> OK v) ==>
        lift_result R_ok R_term r1 (run_blocks fuel' ctx fn' s2)) /\
      (!v. r1 = OK v ==>
        lift_result R_ok R_term (Halt v) (run_blocks fuel' ctx fn' s2))
Proof
  rpt strip_tac >>
  ...
  >- (
    ...
    >- (
      ...
      `run_blocks (SUC f) ctx fn' s2 = Halt v2` by
        metis_tac[run_blocks_at_ok_halted_block] >>
      simp[terminates_def] >>
...
Triviality fwd_left_block_terminal:
  ...
  ==>
    ?fuel'. terminates (run_blocks fuel' ctx fn' s2) /\
      lift_result R_ok R_term (run_blocks (SUC f) ctx fn s1)
        (run_blocks fuel' ctx fn' s2)
Proof
  rpt strip_tac >>
...
Triviality fwd_left_terminal:
  ...
  ==>
    ?fuel'. terminates (run_blocks fuel' ctx fn2 s2) /\
      lift_result R_ok R_term (run_blocks (SUC f) ctx fn1 s1)
   ...
   (R_term : venom_state -> venom_state -> bool) fn1 fn2 bb s1 s2 f nn ctx.
    ...
    (!nn2 v v'.
       ...
       ?fuel2. terminates (run_blocks fuel2 ctx fn2 v') /\
         lift_result R_ok R_term (run_blocks f ctx fn1 v)
...
Triviality fwd_right_terminal:
  ...
  ==>
    ?fuel'. terminates (run_blocks fuel' ctx fn2 s2) /\
      lift_result R_ok R_term (run_blocks fuel ctx fn1 s1)
        (run_blocks fuel' ctx fn2 s2)
Proof
  rpt strip_tac >>
  ...
  !(R_ok : venom_state -> venom_state -> bool)
   (R_term : venom_state -> venom_state -> bool) fn1 fn2 bb s1 s2 fuel nn ctx.
    ...
    (!v v'.
       ...
       ?fuel2. terminates (run_blocks fuel2 ctx fn2 v') /\
         lift_result R_ok R_term (run_blocks fuel ctx fn1 v)
           (run_blocks fuel2 ctx fn2 v'))
  ==>
    ?fuel'. terminates (run_blocks fuel' ctx fn2 s2) /\
      lift_result R_ok R_term (run_blocks fuel ctx fn1 s1)
